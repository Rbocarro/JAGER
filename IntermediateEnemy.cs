...
namespace JAGER
{
    class IntermediateEnemy
    {
        ...
        public IntermediateEnemy(ContentManager content, Player player)
        {
            ...
            {
                case State.Red:
                    texture = this.content.Load<Texture2D>("sprites/game/intermediate_red");
                    break;
                case State.Blue:
                    texture = this.content.Load<Texture2D>("sprites/game/intermediate_blue");
                    break;
            }
        ...
        public void Update(GameTime gameTime)
        {
            ...
            {
                case State.Red:
                    currentEnemyPos.X += (float)Math.Sin(gameTime.TotalGameTime.TotalSeconds * 5) * 8;
                    currentEnemyPos.Y += (enemySpeed + (float)Math.Cos(gameTime.TotalGameTime.TotalSeconds * 4) * 5);
                    break;
                case State.Blue:
                    currentEnemyPos.X += (float)Math.Cos(gameTime.TotalGameTime.TotalSeconds * 5) * 8;
                    currentEnemyPos.Y += (enemySpeed + (float)Math.Sin(gameTime.TotalGameTime.TotalSeconds * 4) * 5);
                    break;
            }
            ...
            if (currentEnemyPos.Y >= 75 && currentEnemyPos.Y <= 250)
            {
                if (r.Next(1, 11) >= 4) { FireBullet(); }
            }
